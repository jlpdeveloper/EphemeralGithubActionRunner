...
if [ -n "${GITHUB_REPOSITORY}" ]
then
  ./config.sh --unattended --name $RUNNER_NAME --labels "${RUNNER_LABELS}" --token "$(get_token)" --url "${registration_url}" --disableupdate
else
  ./config.sh --unattended --name $RUNNER_NAME --labels "${RUNNER_LABELS}" --token "$(get_token)" --url "${registration_url}" --runnergroup "${RUNNER_GROUP}" --disableupdate
fi
